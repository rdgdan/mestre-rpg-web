...
const LevelUpModal: React.FC<LevelUpModalProps> = ({ isOpen, onClose, onApply, level, charClassName, progression }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/85 backdrop-blur-md flex justify-center items-center z-[100] p-4 animate-in fade-in duration-500">
            <div className={`bg-rpg-panel border-2 ${isMaxLevel ? 'border-purple-500 shadow-[0_0_60px_-10px_rgba(168,85,247,0.6)]' : 'border-rpg-gold/50 shadow-[0_0_50px_-10px_rgba(218,165,32,0.5)]'} rounded-lg w-full max-w-lg overflow-hidden relative group`}>
                ...
                <div className="p-8 text-center bg-black/40 border-b border-rpg-gold/20 relative overflow-hidden">
                    <div className="absolute inset-0 flex justify-center items-center opacity-10">
                        <span className={`text-9xl font-black ${isMaxLevel ? 'text-purple-500' : 'text-rpg-gold'} select-none`}>{level}</span>
                    </div>
                    <h2 className={`${isMaxLevel ? 'text-purple-400' : 'text-rpg-gold'} text-sm font-bold tracking-[0.3em] uppercase mb-2 font-cinzel`}>
                    ...
                    <div className="flex justify-center items-baseline gap-4 mb-4">
                        <span className={`text-6xl font-extrabold ${isMaxLevel ? 'text-purple-200' : 'text-rpg-parchment'} font-cinzel text-shadow-glow`}>{level}</span>
                        <span className="text-2xl font-medieval text-rpg-grey italic">{isMaxLevel ? 'Lenda Viva' : 'nesta jornada'}</span>
                    </div>
                    <p className="text-rpg-gold-light text-xl font-medieval font-bold border-t border-b border-rpg-gold/10 py-2 inline-block px-8">
                        {charClassName}
                    </p>
                </div>

                {/* Conteúdo das Habilidades */}
                <div className="p-6 max-h-[55vh] overflow-y-auto custom-scrollbar relative bg-rpg-panel/80 space-y-8">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {Object.keys(attrChoices).map(attr => (
                                    <div key={attr} className="flex items-center justify-between bg-black/20 p-2 rounded border border-blue-500/10">
                                        <span className="text-[10px] font-bold text-rpg-parchment uppercase">{attr.slice(0, 3)}</span>
                                        <div className="flex items-center gap-3">
                                            <button onClick={() => handleAttrChange(attr, -1)} className="w-6 h-6 flex items-center justify-center bg-rpg-slate rounded hover:bg-rpg-dark text-rpg-gold font-bold transition-colors">-</button>
                                            <span className="w-4 text-center font-bold text-blue-300">{attrChoices[attr]}</span>
                                            <button onClick={() => handleAttrChange(attr, 1)} className="w-6 h-6 flex items-center justify-center bg-rpg-slate rounded hover:bg-rpg-dark text-rpg-gold font-bold transition-colors">+</button>
                                        </div>
                                    </div>
                                ))}
                    ...
                    <div className="bg-rpg-red/5 border border-rpg-red/20 rounded-lg p-5">
                        ...
                        <div className="flex items-center gap-6">
                            ...
                            <div className="text-center bg-black/40 p-4 rounded-lg border border-rpg-red/10 w-32">
                                <span className="text-[10px] text-rpg-grey uppercase block mb-1">Dado</span>
                ...
                {/* Rodapé */}
                <div className="p-6 border-t border-rpg-gold/20 flex justify-center bg-black/40">
                    <button
                        onClick={() => {
                            onApply({ attributes: attrChoices, hpIncrease });
                            onClose();
                        }}
                        className="px-12 py-3 bg-gradient-to-r from-rpg-gold via-yellow-400 to-rpg-gold text-rpg-dark font-black rounded shadow-[0_0_20px_rgba(255,215,0,0.3)] hover:shadow-[0_0_30px_rgba(255,215,0,0.5)] transform hover:scale-105 active:scale-95 transition-all uppercase tracking-widest text-sm font-cinzel"
                    >
                        Reivindicar Poder
                    </button>
                </div>
